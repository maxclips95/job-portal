...
export default function EmployerApplicantsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const appRows = appsResp.data?.data?.applications || [];
  ...
  const rateApplication = async (applicationId: string) => {
    const draft = ratingDrafts[applicationId];
    ...
    try {
      await axios.post(
        `${API_BASE}/applications/${applicationId}/rate`,
        { rating, notes },
        getAuthConfig()
      );
      await load();
    } catch (err: any) {
      setError(err?.response?.data?.message || err.message || 'Failed to rate application');
    } finally {
      setActionLoadingId(null);
    }
  };
                  ...
                  <div className="mt-4 border-t pt-4 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                      <input
                        ...
                        max={5}
                        step={0.5}
                        value={ratingDrafts[app.id]?.rating ?? 0}
                        onChange={(e) =>
                          setRatingDrafts((prev) => ({
                            ...prev,
                            [app.id]: {
                      ...
                      <input
                        type="text"
                        value={ratingDrafts[app.id]?.notes ?? ''}
                        onChange={(e) =>
                          setRatingDrafts((prev) => ({
                            ...prev,
                            [app.id]: {
                              rating: prev[app.id]?.rating ?? 0,
                      ...
                      <button
                        disabled={actionLoadingId === app.id}
                        onClick={() => rateApplication(app.id)}
