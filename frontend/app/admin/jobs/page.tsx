...
export default function JobsPage() {
  ...
  const handleRejectJob = async (jobId: string) => {
    try {
      setProcessing(true);
      const reason = rejectionReason.trim() || 'Rejected by admin after review';
      await axios.put(
            ...
            <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-gray-50 p-6 border-b border-gray-200 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-gray-600">{jobDetails.company || jobDetails.company_name || '-'}</p>
                </div>
                <button
                  onClick={() => {
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">
                      {jobDetails.location || [jobDetails.city, jobDetails.country].filter(Boolean).join(', ') || '-'}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                    ...
                    <p className="text-gray-900">
                      {jobDetails.salary?.currency || jobDetails.salary_currency || 'USD'}{' '}
                      {(jobDetails.salary?.min || jobDetails.salary_min || 0).toLocaleString()} -
                      {(jobDetails.salary?.max || jobDetails.salary_max || 0).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                    ...
                    <p className="text-gray-900">{jobDetails.jobType || jobDetails.job_type || '-'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                      Category
                    </label>
                    <p className="text-gray-900">{jobDetails.category || jobDetails.category_name || '-'}</p>
                  </div>
                </div>
